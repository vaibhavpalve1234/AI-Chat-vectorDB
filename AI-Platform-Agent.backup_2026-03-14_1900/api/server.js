...
app.post('/api/memory', async (req, res) => {
  ...
  try {
    ...
    // Include vector DB readiness so callers aren't surprised when Chroma is offline.
    const stats = await getMemoryStats().catch(() => null);
    ok(res, { id, vectorDB: stats?.vectorDB ?? null });
  } catch (err) { fail(res, err.message, 500); }
});
